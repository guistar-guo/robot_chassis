/**
  ...
  * File Name          : MPU6050.c
  * Description        : 陀螺仪驱动代码
	* @author            : C50C厂家		郭耀辉（负责改进）
  ******************************************************************************
  * @attention
  *
  * 使用例子见mpu6050_get_data函数，位于本文件的最后
  *
  ******************************************************************************
  */
...
void MPU_Get_Accelscope()
{
	...
	if(systick>500 && flag2 == 0)  //开机前0.5秒
		{
			flag1 = 1;
		}
		if(flag1==1 && flag2 == 0)
		...
		accel[0] -= Original_accel[0];
		accel[1] -= Original_accel[1];  
		accel[2] = accel[2] - Original_accel[2]+16384;//16384是重力加速度g对应的原始数据
...
u8 MPU_Get_Gyroscope()
{
	...
	if(systick>500 && flag2 == 0)  //开机前0.5秒
		...
		gyro[0] -= Original_gyro[0];
		gyro[1] -= Original_gyro[1];  
		gyro[2] -= Original_gyro[2];
